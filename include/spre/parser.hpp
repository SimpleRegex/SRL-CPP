...
namespace spre
{
	...
	inline unique_ptr<CharacterExprAST> Parser::parse_character(const TokenValue &token_value)
	{
		...
		if (token_value == TokenValue::LITERALLY
			|| token_value == TokenValue::ONE_OF
			|| token_value == TokenValue::RAW)
		{
		...
		else if (token_value == TokenValue::LETTER
			|| token_value == TokenValue::UPPERCASE_LETTER
			|| token_value == TokenValue::DIGIT)
		{
			Token next_token = lexer_.get_next_token();
	...
	inline unique_ptr<QuantifierExprAST> Parser::parse_quantifier(const TokenValue &token_value)
	{
		...
		case TokenValue::EXCATLY_X_TIMES:
		{
			Token next_token = lexer_.get_next_token();
			Token next_next_token = lexer_.get_next_token();
			if (next_token.get_token_value() == TokenValue::NUMBER
				&& (next_next_token.get_token_value() == TokenValue::TIME
					|| next_next_token.get_token_value() == TokenValue::TIMES))
			{
		...
		case TokenValue::BETWEEN_X_AND_Y_TIMES:
		{
			...
			if (x.get_token_value() == TokenValue::NUMBER
				&& and_token.get_token_value() == TokenValue::AND
				&& y.get_token_value() == TokenValue::NUMBER)
			{
				string val = "{";
		...
		case TokenValue::AT_LEAST_X_TIMES:
		{
			...
			Token times = lexer_.get_next_token();
			if (x.get_token_value() == TokenValue::NUMBER
				&& times.get_token_value() == TokenValue::TIMES)
			{
	...
	inline unique_ptr<GroupExprAST> Parser::parse_group(const TokenValue &token_value)
	{
		...
		case TokenValue::CAPTURE_AS:
		{
			...
			if (group_start.get_token_value() == TokenValue::GROUP_START
				&& group_end.get_token_value() == TokenValue::GROUP_END)
			{
